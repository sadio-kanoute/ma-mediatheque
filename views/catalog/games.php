...
<head>
    ...
    <title><?php echo e($title ?? 'Jeux Vidéo'); ?></title>
    ...
    <section class="container">
            ...
            <section class="search-filters">
                ...
                <form method="GET" action="<?php echo url('catalog/games'); ?>">
                    ...
                    <div class="search-bar">
                        ...
                        <input type="text" name="search_term" placeholder="Rechercher un jeu vidéo..." class="search-input" value="<?php echo e($search_term ?? ''); ?>">
                        ...
                        <?php if (!empty($search_term) || ($search_genre ?? 'all') != 'all' || ($search_availability ?? 'all') != 'all'): ?>
                            ...
                            <a href="<?php echo url('catalog/games'); ?>" class="btn-effacer">Effacer</a>
                        <?php endif; ?>
                    </div>
            ...
            <?php else: ?>
                <?php foreach ($items as $item): ?>
                    <div class="item">
                        ...
                        <p>Plate-forme : <?php echo e($item['isbn_rating_platform']); ?></p>
        ...
        <div class="pagination">
            ...
            <?php if ($current_page > 1): ?>
                <a href="<?php echo url('catalog/games?page=' . ($current_page - 1) . '&search_term=' . urlencode($search_term) . '&genre=' . ($search_genre ?? 'all') . '&availability=' . ($search_availability ?? 'all')); ?>" class="btn btn-prev-page">Page précédente</a>
            <?php endif; ?>

            <!-- Liens numériques des pages -->
            <div class="page-numbers">
                <!-- Toujours afficher la page 1 -->
                <a href="<?php echo url('catalog/games?page=1&search_term=' . urlencode($search_term) . '&genre=' . ($search_genre ?? 'all') . '&availability=' . ($search_availability ?? 'all')); ?>" class="page-number <?php echo $current_page == 1 ? 'active' : ''; ?>">1</a>
                ...
                for ($i = $start_page; $i <= $end_page; $i++): ?>
                    <a href="<?php echo url('catalog/games?page=' . $i . '&search_term=' . urlencode($search_term) . '&genre=' . ($search_genre ?? 'all') . '&availability=' . ($search_availability ?? 'all')); ?>" class="page-number <?php echo $i == $current_page ? 'active' : ''; ?>">
                        <?php echo $i; ?>
                    </a>
                <?php endfor; ?>
            ...
            <?php if ($current_page < $total_pages): ?>
                <a href="<?php echo url('catalog/games?page=' . ($current_page + 1) . '&search_term=' . urlencode($search_term) . '&genre=' . ($search_genre ?? 'all') . '&availability=' . ($search_availability ?? 'all')); ?>" class="btn btn-next-page">Page suivante</a>
            <?php endif; ?>
